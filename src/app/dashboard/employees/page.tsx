...
function EmployeesPage() {
  ...
  const addEmployee = async () => {
    if (!name || !email) {
      ...
      setAlertMessage("Bitte alle Felder ausfüllen");
      setShowAlert(true);
      return;
    }
    ...
    if (insertError) {
      console.error("Fehler beim Hinzufügen des Mitarbeiters:", insertError);
      setAlertType("error");
      setAlertMessage("Fehler beim Hinzufügen des Mitarbeiters");
      setShowAlert(true);
      return;
    }
    ...
    if (inviteError) {
      console.error("Fehler beim Senden des Magic-Links:", inviteError);
      setAlertType("error");
      setAlertMessage("Mitarbeiter hinzugefügt, aber Magic‑Link nicht gesendet");
      setShowAlert(true);
    } else {
      setAlertType("success");
      setAlertMessage("Mitarbeiter hinzugefügt und Magic‑Link per E‑Mail gesendet!");
      setShowAlert(true);
    }
  ...
  const saveEdit = async () => {
    ...
    if (error) {
      ...
      setAlertType("error");
      setAlertMessage("Fehler beim Aktualisieren des Mitarbeiters");
      setShowAlert(true);
    } else {
      fetchEmployees();
      cancelEditing();
      setAlertType("success");
      setAlertMessage("Mitarbeiter erfolgreich aktualisiert!");
      setShowAlert(true);
    }
  };
  ...
  const getRoleInfo = (rolle: string) => {
    switch (rolle) {
      ...
      case 'waiter':
        return { color: 'bg-green-100 text-green-800 border-green-200', label: 'Kellner' };
      default:
        return { color: 'bg-gray-100 text-gray-800 border-gray-200', label: 'Unbekannt' };
    }
  };
  ...
  const getStatusInfo = (status: string) => {
    switch (status) {
      case 'active':
        return { color: 'bg-green-100 text-green-800 border-green-200', label: 'Aktiv' };
      case 'pending':
        return { color: 'bg-yellow-100 text-yellow-800 border-yellow-200', label: 'Ausstehend' };
      case 'inactive':
        return { color: 'bg-red-100 text-red-800 border-red-200', label: 'Inaktiv' };
      default:
        return { color: 'bg-gray-100 text-gray-800 border-gray-200', label: 'Unbekannt' };
    }
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Mitarbeiterkonten und Berechtigungen verwalten
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <p className="text-sm font-medium text-gray-600">Gesamtanzahl Mitarbeiter</p>
          <p className="text-2xl font-bold text-gray-900 mt-2">{employees.length}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <p className="text-sm font-medium text-gray-600">Kellner</p>
        ...
        <div className="fixed inset-0 bg-gray-700/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-900">Neuen Mitarbeiter hinzufügen</h3>
              <button
      ...
      {deleteId && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            if (error) {
              setAlertType("error");
              setAlertMessage("Fehler beim Löschen des Mitarbeiters");
              setShowAlert(true);
            } else {
              setEmployees((prev) => prev.filter((emp) => emp.id !== deleteId));
              setAlertType("success");
              setAlertMessage("Mitarbeiter erfolgreich gelöscht");
              setShowAlert(true);
            }
          }}
        />
        ...
        {filteredEmployees.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-gray-500">
              {employees.length === 0 ? 'Keine Mitarbeiter gefunden.' : 'Keine Mitarbeiter entsprechend Ihrer Kriterien gefunden.'}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">Mitarbeiter</th>
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">Rolle</th>
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">Status</th>
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-700">Aktionen</th>
                </tr>
              </thead>
